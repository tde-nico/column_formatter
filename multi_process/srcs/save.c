...
int	alloc_form(t_formatter *f)
{
	...
	while (f->cols.cols[i] != NULL)
		++i;
	size = 1 + (f->height + 1) * (i / (f->height * f->max));
	i %= (f->height * f->max);
...
*/
int	save_first_line(t_formatter *f, int i)
{
	size_t	form_index;
	size_t	col_index;
...
/**
 * @brief adds the page separator to the output
 * @param f the formatter struct
 * @param i the current formatted line to parse
 * @return 0 by default, 1 if error
 * 
 * it appends a page separator between pages (the "page separator" string can
 * be modified by editing the macro)
*/
int	save_page_separator(t_formatter *f, int *i)
{
	if ((*i + 1) / f->height >= f->max)
	{
		*i = -1;
		...
		if (f->cols.form[(f->cols.curr) * (f->height + 1) - 1] == NULL)
			return (raise_error_i("malloc error", f));
	}
	return (0);
}
...
int	save_columns(t_formatter *f)
{
	...
	while (f->cols.cols[++i + f->cols.curr * (f->max * f->height)] != NULL)
	{
		...
		if (save_column(f, i) || save_page_separator(f, &i))
			return (1);
	}
	return (0);
}
